...
//
// Max achieved speed: 20000 read from 8 channels per second 
// Achieved 160kSPS; ADC Hardware Max 500kSPS
//
...
// The result is 0 when the device is not actively converting (Incorrect order in setup)
// ===========================================================================

uint16_t ADS8688::noOpRaw() {
    return cmdRegister(NO_OP, true);
}
...
// Default power-on value = 0x00 (all channels powered up).
...
// ===========================================================================
// PUBLIC METHODS - FEATURE SELECT  (Description by Claude)
//
// Register layout:
...
// ===========================================================================
// PUBLIC METHODS - ALARM FLAG REGISTERS (Description by Claude)
// ===========================================================================
...
// ===========================================================================
// PUBLIC METHODS - ALARM THRESHOLDS (Description by Claude)
// Register base address for channel n: CH0_base + 5*n
// ===========================================================================
...
// ===========================================================================
// PUBLIC METHODS (Newly added functions)
// ===========================================================================

// The hardware can do up to 500 kSPS.
// 0 to disable rate limiting.
void ADS8688::setSampleRate(uint32_t sps) {
    if (sps == 0) {
        _samplePeriodUs = 0;
    } else {
        if (sps > 500000UL) sps = 500000UL;   // clamp to max
...
// lastSampleUs set in cmdRegister after SPI transaction has ended
...
// ===========================================================================
// PRIVATE METHODS - SPI TRANSACTIONS (Modified by Claude + myself)
// ===========================================================================
...
uint8_t ADS8688::readRegister(uint8_t reg) {
    ...
    tx[2] = 0x00;
        ...
        uint8_t result = ((byte2 & 0x01) << 7) | (byte3 >> 1);
...
uint16_t ADS8688::cmdRegister(uint8_t reg, bool manual) {

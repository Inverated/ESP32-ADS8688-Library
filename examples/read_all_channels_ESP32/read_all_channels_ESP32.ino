...
void setup() {
    ...
    for (int ch = 0; ch < 8; ch++) {
        uint16_t raw = adc.noOp();
        Serial.printf("CH%d raw = %u (0x%04X)\n", ch, raw, raw);
    }
...
void loop() {
    ...
    // Wait for next aggregate sample slot
    adc.waitForSample();
    for (int i = 0; i < sizeof(r1_pins) / sizeof(r1_pins[0]); i++) {
    ...
    if (count % 10000 == 0) {
        ...
        //Serial.printf("Count = %d, currTime passed since last (s) = %.5f\n", count, (now - currTime)/1000.0);
        currTime = now;
    }
